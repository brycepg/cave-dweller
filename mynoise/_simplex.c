...
double fbm_noise4_mod(float x, float y, float z, int octaves)
{
    /* z == seed */
	const float persistence = 0.5f;
    const float lacunarity = 2.0f;
    const float repeatx = 65536;
    const float repeaty = 65536;
...
static PyObject*
py_gen_block(PyObject *self, PyObject *args, PyObject *kwargs) {
    float z; /* seed */
    int map_size, idx, idy;
    const float scaling_factor=0.0078125;
    const int octaves = 8;

    if (!PyArg_ParseTuple(args, "fiii", &z, &idx, &idy, &map_size)) {
        return NULL;
    }
    ...
    PyObject *list_main = PyList_New(map_size);
    for(int x=0; x<map_size; x++) {
        PyObject *list_sub = PyList_New(map_size);
        for(int y=0; y<map_size; y++) {
            PyList_SetItem(list_sub, y, PyFloat_FromDouble(fbm_noise4_mod((idx_abs_base + x)*scaling_factor, (idy_abs_base + y)*scaling_factor, z, octaves)));
        }
        PyList_SetItem(list_main, x, list_sub);
...
static PyMethodDef simplex_functions[] = {
	...
	{"noise4", (PyCFunction)py_noise4, METH_VARARGS | METH_KEYWORDS, 
        ...
        "lacunarity -- specifies the frequency of each successive octave relative\n"
        "to the one below it, similar to persistence. Defaults to 2.0."},
    {"gen_block", (PyCFunction)py_gen_block, METH_VARARGS,
        "gen_block(seed, idx, idy, map_size=96, octaves=8)"},
	{NULL}
};

/*static * PyObject
py_gen_block(PyObject *self, PyObject *args, PyObject *kwargs) {
    const float scaling_factor = 1f/128f;
    int idx, idy;
    float z;
    int octaves, map_size; 
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "fff|ii:gen_block", kwlist,
		&z, &idx, &idy, &octaves, &map_size))
		return NULL;
    int map_area = map_size * map_size;
    int idx_abs_base = idx*map_size;
    float* map_array = malloc(sizeof *map_array * map_area);
    for(int i = 0; i < map_area; i++) {
        map_array[i] = fbm_noise4_mod((idx * map_size (i % map_size))*scaling_factor, y*scaling_factor, z, octaves)
    }
}*/
